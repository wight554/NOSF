#!/usr/bin/env python3
"""
NOSF — StallGuard Auto-Tuner (Proactive Setup)
Automatically configures the controller for StallGuard-dominant tuning.
"""
...
def run_collection(args, ser, baseline=None):
    ...
    # 1. Proactive Setup for Tuning
    print("[*] Proactively configuring controller for tuning session...")
    send_wait(ser, "SET:SYNC_SG_INTERP:1")  # Force StallGuard-based sync
    send_wait(ser, "SM:1")                   # Enable sync mode (host override)
    
    # Apply baseline settings if available
    if baseline:
        for k in ['driver_tbl', 'driver_toff', 'driver_hstrt', 'driver_hend']:
            if k in baseline: send_wait(ser, f"SET:{k.upper()}:{baseline[k]}")
        # Use baseline SG current if provided, otherwise default to 800mA for consistency
    ...
    print(f"[*] Sweep: [{sgt_min}, {sgt_max}] every {args.step_interval}s")
