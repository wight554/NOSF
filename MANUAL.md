...
| `SYNC_AUTO_STOP` | `sync_auto_stop_ms` | Auto-mode only: tail-assist stop after sustained `TRAILING`, and normal print-sync stop only after `TRAILING` recovery has already collapsed to the minimum sync floor for X ms while the MMU is still positively pushing into trailing | 5000 |
