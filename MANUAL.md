...
| `SYNC_RESERVE_PCT` | `sync_reserve_pct` | Normal-sync reserve target as % of `BUF_HALF_TRAVEL` toward trailing | 30 |
...
| `SYNC_AUTO_STOP` | `sync_auto_stop_ms` | Auto-mode only: tail-assist timeout; disable the auto-started tail-clear assist after sustained `TRAILING` for X ms | 5000 |
