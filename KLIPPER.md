# NightOwl — Klipper Integration

This document covers connecting Klipper to the NightOwl controller: serial
setup, the shell command helper, toolhead sensor and toolchange macros, and the
advance profiling workflow for sync tuning.

For the NightOwl command reference see `MANUAL.md`; for behavioral details see
`BEHAVIOR.md`.
...
The NightOwl controller appears as a USB CDC serial device on the Raspberry Pi
...
## Shell command helper — nightowl_cmd.py

`scripts/nightowl_cmd.py` sends a single NightOwl command and blocks until the
...
Add to `printer.cfg`:
```ini
[gcode_shell_command nightowl]
command: python3 /home/pi/nightowl-standalone-controller/scripts/nightowl_cmd.py
timeout: 130.0
verbose: True
```
...
```
RUN_SHELL_COMMAND CMD=nightowl PARAMS="?:"
```
...
NightOwl needs to know when filament reaches or leaves the toolhead extruder.
...
insert_gcode:
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="TS:1"
runout_gcode:
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="TS:0"
```

### Option B — Buffer fallback (no sensor)

When filament presses against the extruder gears, the buffer arm holds TRAILING
for `TS_BUF_MS` milliseconds and NightOwl self-triggers the loaded state.
...
lane, and waits for `TS:1`.  `nightowl_cmd.py` blocks until `EV:TC:DONE` or
...
gcode:
    ...
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="TC:1"
...
gcode:
    M400
    SAVE_GCODE_STATE NAME=_tc_state
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="TC:2"
    RESTORE_GCODE_STATE NAME=_tc_state
```
...
If `TC:` returns an error, `nightowl_cmd.py` exits with code 1.
...
gcode:
    ...
    SAVE_GCODE_STATE NAME=_tc_state
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="TC:1"
    {% if printer['gcode_shell_command nightowl'].return_code != 0 %}
        PAUSE
        { action_respond_info("NightOwl TC:1 failed") }
    {% endif %}
    RESTORE_GCODE_STATE NAME=_tc_state
```
...
```ini
[gcode_macro NIGHTOWL_LOAD]
description: Full load active lane to toolhead
gcode:
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="FL:"

[gcode_macro NIGHTOWL_UNLOAD]
description: Unload from extruder (tip past OUT sensor)
gcode:
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="UL:"

[gcode_macro NIGHTOWL_PRELOAD]
description: Pre-load active lane to parked position (OUT sensor)
gcode:
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="LO:"
```
...
gcode:
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="SM:0"

[gcode_macro SYNC_ON]
gcode:
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="SM:1"
```
...
The `NIGHTOWL_ADVANCE_PROFILE` macro drives the MMU motor and the extruder in
...
gcode:
    ...
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="SG:{LANE}"   ; capture SG under tension
    G1 E50 F2400                                        ; 40 mm/s — second half
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="SG:{LANE}"   ; capture SG again
    ; Stop
    RUN_SHELL_COMMAND CMD=nightowl PARAMS="ST:"
    { action_respond_info("Profiling complete. Use the lowest SG value above as SG_TENSION_MAX.") }
    { action_respond_info("Apply: python3 scripts/nightowl_test.py SET:SG_TENSION_MAX:<value> SV:") }
```

Run from the Mainsail / Fluidd console:
```
NIGHTOWL_ADVANCE_PROFILE LANE=1
```

The two `SG:` reads appear in the console output.  The lower of the two values
is your `SG_TENSION_MAX`.  Apply it:
```
RUN_SHELL_COMMAND CMD=nightowl PARAMS="SET:SG_TENSION_MAX:24"
RUN_SHELL_COMMAND CMD=nightowl PARAMS="SV:"
```
...
|---------|--------------|-----|
| `nightowl_cmd.py` exits "no serial port found" | Port not present | `ls /dev/ttyACM*`; check `dialout` group |
| `TS:1` not reaching NightOwl | Sensor wiring or config | Test: `RUN_SHELL_COMMAND CMD=nightowl PARAMS="TS:1"` |
