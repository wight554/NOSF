...
- 2.12.2 done: added analyzer LOCKED-bucket floor, safe/aggressive/force mode semantics, removed `SAFETY_K` subtraction from baseline, rewrote confidence to bucket-evidence labels, and added analyzer tests. Validation passed (`python3 -m py_compile scripts/nosf_analyze.py`, `python3 -m py_compile scripts/test_nosf_analyze.py`, `python3 scripts/test_nosf_analyze.py`, `python3 scripts/test_nosf_live_tuner.py`, `python3 -m py_compile scripts/*.py`). Commit SHA reported after push.
