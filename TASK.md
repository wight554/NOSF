...
- 2.12.3 done: replaced dominant-bucket baseline and all-row bias with trimmed precision-weighted qualifying-bucket aggregation; added field oscillation fixtures and tests. Validation passed (`python3 -m py_compile scripts/nosf_analyze.py`, `python3 -m py_compile scripts/test_nosf_analyze.py`, `python3 scripts/test_nosf_analyze.py`, `python3 scripts/test_nosf_live_tuner.py`, `python3 -m py_compile scripts/*.py`). Commit SHA reported after push.
