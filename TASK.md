...
- 2.11.2 done: added residual EWMA bucket fields, schema 3→4 migration, schema-chain tests, and analyzer schema-4 load compatibility while leaving lock/unlock behavior unchanged; validation passed (`python3 -m py_compile scripts/nosf_live_tuner.py`, `python3 -m py_compile scripts/test_nosf_live_tuner.py`, `python3 scripts/test_nosf_live_tuner.py`, `python3 scripts/test_phase_2_10_parity.py`, `python3 scripts/test_klipper_motion_tracker.py`, `python3 -m py_compile scripts/*.py`). Commit SHA reported after push.
