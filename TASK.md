...
- 2.11.3 done: replaced single-sample unlock with catastrophic/streak/drift channels, added noise-gated locking and lock dwell, flipped chatter repro to a hard assertion, and added algorithm tests; validation passed (`python3 -m py_compile scripts/nosf_live_tuner.py`, `python3 -m py_compile scripts/test_nosf_live_tuner.py`, `python3 scripts/test_nosf_live_tuner.py`, `python3 scripts/test_phase_2_10_parity.py`, `python3 scripts/test_klipper_motion_tracker.py`, `python3 -m py_compile scripts/*.py`). Commit SHA reported after push.
