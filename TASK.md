...
- 2.12.5 done: added `[nosf_contributors]` patch diagnostics and documented Phase 2.12 analyzer modes, `--force`, relative noise wait reason, and contributor interpretation. Validation passed (`python3 scripts/test_nosf_analyze.py`, `python3 scripts/test_nosf_live_tuner.py`, `python3 -m py_compile scripts/*.py`). Commit SHA reported after push.
